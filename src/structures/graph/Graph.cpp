...
namespace procedural {
  ...
  Graph::Graph(const std::string& name, int64_t id, const std::string& type_str) : type_id_(graph_table.get(type_str_)), name_(name), id_(id),
                                                                                   type_str_(type_str)
  {
...
//      LOG_DEBUG << "No match found for observation: " + observation->toString();
  ...
  uint64_t Graph::getMaxIdAtDepth(int depth) const
  {
    uint64_t maxId = 0;
    for(const auto& pair : nodes_)
    {
      if(pair.second->getDepth() == depth && pair.second->getId() > maxId)
      {
        maxId = pair.second->getId();
      }
    }
    return maxId;
  }

  std::unordered_set<uint64_t> Graph::getIdsAtDepth(int depth) const
  {
    std::unordered_set<uint64_t> ids;
    for(const auto& pair : nodes_)
    {
      if(pair.second->getDepth() == depth)
      {
        ids.insert(pair.second->getId());
      }
    }
    return ids;
  }

  int Graph::getIdAtDepthThatSatisfyConstraint(int depth, std::shared_ptr<Transition> Transition,
                                               const std::set<int>& const_source_node) const
  {
    auto nodes_same_depth = getIdsAtDepth(depth);
    for(const auto& id : nodes_same_depth)
    {
      if(nodes_.at(id)->satisfyTransitionConstraint(Transition))
      {
        if(std::includes(nodes_.at(id)->getSatisfyConstraint().begin(),
                         nodes_.at(id)->getSatisfyConstraint().end(), const_source_node.begin(),
                         const_source_node.end()))
        {
          if(nodes_.at(id)->getSatisfyConstraint().find(Transition->getHtnId()) !=
             nodes_.at(id)->getSatisfyConstraint().end())
          {
            return int(id);
          }
        }
